...
class SpockTests extends Specification  {
  ...
  базаДаних.withTransaction {
    ...
    10.times {
      акт.rows << new Row(name: "name${it}", amount: it,  owner: акт)
    }
  ...
  базаДаних.findAll(Акт)[0].sum == 45
  }
  ...
  базаДаних.withTransaction {
    ...
    Накладна накладна = Документ.новий(Накладна, [рахунокДебет: дебет, рахунокКредит: кредит])

    накладна.rows << new Row(name: 'row1', amount: 10.0f, owner: накладна)
  ...
  базаДаних.withTransaction {
    ...
    прихіднаНакладна.rows << new Row(name: 'row1', amount: 20.0f, owner: прихіднаНакладна)
    ...
    розхіднаНакладна.rows << new Row(name: 'row1', amount: 30.0f, owner: розхіднаНакладна)
